...
- **PMPlayerModel**: Hosts the local-identity getters. Signatures (from the GUI Object Dumper, in-match):
  ...
  - **Lua-side reachability (this UE4SS build, v3.0.1):** The two `GetCached*V1` UFunctions above are **not callable from Lua** despite the signatures being correct — every documented placeholder shape (`(false, nil)`, `(false, {})`, `(false)`, `()`) errors at the marshaling layer. This is a UE4SS limitation on the `(Bool out, X out)` shape, not a UE limitation. Working path is the delegate property `GetMeRequestV1Completed` bound via a ModActor BP wrapper (Lua's `prop:Add` takes `(UObject, FName)`, not a Lua function — passing one is a native crash that `pcall` cannot catch). See `docs/learnings/ue4ss-outparam-marshaling-failure.md` and `docs/learnings/ue4ss-lua-multicast-delegate-binding.md`.
