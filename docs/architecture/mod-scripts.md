...
| `chat.lua` | Feature: in-match chat | Finds the on-screen chat widget, owns Enter/Esc keybinds and the `OnRep_MatchState` hook (registered in its own `M.init()`), formats messages, derives the match-wide room code from the match seed, parses chat audiences (`team`, `all`, `Team 1`, `Team 2`), tracks presence. |
