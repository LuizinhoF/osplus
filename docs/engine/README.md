...
| [`ue4ss-version-and-gotchas.md`](./ue4ss-version-and-gotchas.md) | **migrated** (batch 1, 2026-05-01) | 3.0.1 anchor, Lua API, common pitfalls, version-sensitive bugs (out-param marshaling, multicast delegate no-op, `ExecuteInGameThread` corruption, first-registration-wins custom events, etc.) |
| [`widgets.md`](./widgets.md) | **migrated** (batch 1, 2026-05-01) | UMG-only HUD model, BPModLoaderMod lifecycle, asset loading, actor spawning, material setup, widget catalog, ScrollBox crash root-cause, EditableText/Input Mode/Visibility quirks, GameInstance persistence, native Home Hub hierarchy/display-state integration, bounded release links and child hit testing, stale-`GetClass()` safety, the game's own widget tree |
| [`game-state.md`](./game-state.md) | **corrected** (2026-09-05; runtime phase checks pending) | Seed-based match identity, stored `CurrentMatchPhase` / `EMatchPhase` schema, historical class tuples, and the separate pregame presence boundary |
