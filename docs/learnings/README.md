...
|---|---|---|---|
| 2026-04-25 | [ue4ss-outparam-marshaling-failure](ue4ss-outparam-marshaling-failure.md) | re | UE4SS v3.0.1 cannot call `(Bool out, X out)` UFunctions from Lua at all — every documented placeholder shape errors at the marshaling layer. Affects `PMPlayerModel:GetCached*V1` family. Workarounds: BP wrapper, direct UProperty read, UE4SS upgrade. |
| 2026-04-25 | [ue4ss-lua-multicast-delegate-binding](ue4ss-lua-multicast-delegate-binding.md) | re | `MulticastDelegateProperty:Add` takes `(UObject, FName)` — passing a Lua function is a native crash that `pcall` cannot catch. UE4SS userdata `__index` returns placeholder `userdata` for unknown keys (false-friend trap); only six method names resolve to real `function` types. Spikes touching unfamiliar native APIs need persistent crash-survivable logging with `flush()` per call. |
| 2026-04-24 | [os-runtime-data-model](os-runtime-data-model.md) | re | The local Prometheus ID is reachable via `PMPlayerModel:GetMeRequestV1Completed` (delegate, requires ModActor BP wrapper per Pass-4 spike); `MeResponseV1` extends `PlayerPublicProfile` so one fire yields the full profile struct. Per-match counters (Redirects, ShotsOnGoal, Damage, PowerUps) live on `PMPlayerMatchSummary`; the puck is internally called "Rock". |
