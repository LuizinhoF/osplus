...
|---|---|---|---|
| 2026-04-24 | [os-runtime-data-model](os-runtime-data-model.md) | re | The local Prometheus ID is reachable via `PMPlayerModel:GetCachedMeResponseV1` (UE4SS UFunctions need output-param placeholders); `MeResponseV1` extends `PlayerPublicProfile` so one sync call yields the full profile struct. Per-match counters (Redirects, ShotsOnGoal, Damage, PowerUps) live on `PMPlayerMatchSummary`; the puck is internally called "Rock". |
