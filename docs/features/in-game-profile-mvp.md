...
|---|---|---|---|---|
| 3 | `PMPlayerModel` getter UFunctions resolvable to a clean local-Prometheus-ID path | Med-High (callable, signatures unread) | Signatures fully read from dumper; cross-checked in-game via C2 (`ForEachProperty`). `MeResponseV1` ScriptStruct inherits from `PlayerPublicProfile` (`sps` chain). | **High for the *delegate* path, Falsified for the *sync* path.** Pass 4 found `(Bool out, X out)` UFunctions unreachable in this UE4SS build (v3.0.1); delegate path validated via ModActor BP wrapper. See Pass 4 findings section + `docs/learnings/ue4ss-outparam-marshaling-failure.md` + `docs/learnings/ue4ss-lua-multicast-delegate-binding.md`. |
...
**Implications for ADRs and Stage 5:**

- **ADR 0001 is `accepted` on the R-B path with revised cold-start posture** — the warm-cache pre-check is removed from R-B's design; cold start = wait for the natural login fire (~RTT seconds at session start, not user-perceptible at the Stage-5 use-site of "have identity ready by the first IPC handshake"). Path A was chosen explicitly: pay the BP-wrapper cost once, reuse the substrate forever. R-A and R-C inherit the same UFunction-marshaling block on their warm-cache halves and are now strictly worse choices, not just less reactive ones.
...
### Recommended Stage 5 path (revised post-Pass-4)

- **Identity binding:** `full feature` path. Substrate is fully characterized end-to-end via the Pass-4 spike. Build sequence:
  1. **ModActor BP** (proposed slug `BP_OSPlusDelegateBridge`) cooked into the OSPlus pak via the existing pipeline (`.cursor/skills/ue4ss-modding/references/mod-actor-pattern.md`). Class exposes one UFunction `OnMeResponse(Succeeded: Bool, RequestId: Str, MeResponse: MeResponseV1, ErrorResponse: ErrorResponse)` matching `MeRequestV1Completed__DelegateSignature`, plus a Lua-readable forwarding mechanism (specific shape — write to a watched property, fire a sentinel UFunction Lua hooks, etc. — chosen during Stage 4 design).
  2. **`mod/OSPlus/scripts/identity.lua` extension**: acquire the spawned ModActor instance, call `model.GetMeRequestV1Completed:Add(modActor, "OnMeResponse")` once at module init, on bridge fire surface the Prometheus ID alongside the existing SteamID + display-name reads. No polling, no warm-cache pre-check.
  3. **BPModLoaderMod glue** for the new BP class (existing pipeline; no architectural change).
- **Raw capture pipeline:** `thin slice` (unchanged from Pass-3 recommendation). The capture surface is `FindAllOf("PMPlayerMatchSummary")` during a match. Thin slice = (1) read all summaries at end-of-match, (2) write each as one row to wherever `0002-profile-storage` lands them, (3) prove round-trip end-to-end with one match's worth of redirect counts. The per-summary → per-player mapping question is the only thing that could turn this back into a spike. **Note:** if Stage 4 design wants to capture per-event redirects (vs. per-match aggregates), the `PMRockCharacter:LastRedirectKnockBack` delegate path can reuse the same ModActor + bridge substrate built for identity — reducing the marginal cost of per-event capture to "one new BP UFunction matching the new signature + one new `prop:Add` call from Lua."
- **Storage:** still waits for `0002-profile-storage` ADR. Pass-3 findings (canonical `PlayerPublicProfile` shape, bounded `EPMEndOfGameStat` enum) feed concrete schema inputs; Pass-4 findings don't change the storage axis but do reinforce that profile-row reads are event-driven (no synchronous re-fetch on demand without a workaround) — the storage layer should not assume "I can always re-read the local profile from the game on demand."
...
- ~~*[Pass 4]* **Delegate-binding spike (1–2 hours).**~~ — **Resolved in Pass 4 spike.** UE4SS's `MulticastDelegateProperty:Add` takes `(UObject, FName)`, requires a ModActor BP wrapper as the binding target. Full pattern + false-friend trap in `docs/learnings/ue4ss-lua-multicast-delegate-binding.md`. R-B's substrate exists; ADR 0001 is `accepted`.
- *[Pass 5 / Stage 4 design — deferred from Pass 4]* Property-dump probe on `PMPlayerModel` to look for direct UProperty cache fields (e.g., `CachedMeResponse : MeResponseV1`). If such fields exist, they're a zero-build-cost workaround for the synchronous warm-cache read that the UFunction path can't deliver. Not blocking the current Stage 5 path (cold-start delegate fire is sufficient); worth one probe pass when the next feature wants synchronous local-cache reads.
