...
|---|---|---|
| `building` | [`update-availability-notification`](./update-availability-notification.md) | Notify players with a quiet, non-blocking in-game notice when a newer stable OSPlus release is available, checking at startup, matchmaking queue entry, and match completion. |
